...
def _run_trained_attack(attack_input: AttackInputData,
    ...
    elif attack_type == AttackType.SVM_ATTACK:
        attacker = models.SVMAttacker()
    else:
    ...
    gt = prepared_attacker_data.is_training_labels_test

    roc_curve = metric_attacker_test(gt, predictions_test)
...
def _run_threshold_attack(attack_input: AttackInputData):
    gt = np.concatenate((np.zeros(attack_input.get_train_size()),
                         np.ones(attack_input.get_test_size())))
    predicts = np.concatenate(
        (attack_input.get_loss_train(), attack_input.get_loss_test()))
    roc_curve = metric_attacker_test(gt, predicts)

    return SingleAttackResult(
        slice_spec=_get_slice_spec(attack_input),
        data_size=DataSize(ntrain=attack_input.get_train_size(), ntest=attack_input.get_test_size()),
...
def _run_threshold_entropy_attack(attack_input: AttackInputData):
    ...
    # fpr, tpr, thresholds = metrics.roc_curve(gt, predicts)
    #
    # accuracy = metrics.accuracy_score(gt, predicts > 0.5)
    # precision = (tpr * np.sum(gt)) / np.sum(
    #     predicts.reshape((-1, 1)) >= thresholds.reshape((1, -1)), 0)
    # roc_curve = RocCurve(tpr=tpr, fpr=fpr, thresholds=thresholds, precision=precision)
    # AP = metrics.average_precision_score(gt, predicts)
    roc_curve = metric_attacker_test(gt, predicts)
    return SingleAttackResult(
        slice_spec=_get_slice_spec(attack_input),
        data_size=DataSize(ntrain=attack_input.get_train_size(), ntest=attack_input.get_test_size()),
...
def _run_attack(attack_input: AttackInputData,
    ...
    if min(attack_input.get_train_size(),
        ...
        return None
    if attack_type == AttackType.META_ATTACK:
        return _run_meta_attack(attack_input, balance_attacker_training)
...
def metric_attacker_test(gt, pred):
    fpr, tpr, thresholds = metrics.roc_curve(gt, pred)

    precision = (tpr * np.sum(gt)) / np.sum(
        pred.reshape((-1, 1)) >= thresholds.reshape((1, -1)), 0)
    for i in range(precision.shape[0]):
        if math.isnan(precision[i]):
            precision[i] = 0
    AP = metrics.average_precision_score(gt, pred)
    accuracy = np.zeros_like(precision)
    for i in range(len(thresholds)):
        accuracy[i] = metrics.accuracy_score(gt, pred >= thresholds[i])
    roc_curve = RocCurve(tpr=tpr, fpr=fpr, thresholds=thresholds, precision=precision, AP=AP, accuracy=accuracy)
    return roc_curve

def _run_meta_attack(attack_input: AttackInputData,
                        balance_attacker_training: bool = True):
    """
    run meta attack and also other tf attacks (Logistic regression, MLP, randorm forest, SVM, GPC, Decision tree, Adaboost)
    """
    attack_results = []
    prepared_attacker_data = models.create_attacker_data(
        attack_input, balance=balance_attacker_training)
    ntrain = len(prepared_attacker_data.is_training_labels_train)
    ntest = len(prepared_attacker_data.is_training_labels_test)
    meta_train_data = np.zeros((ntrain, 8))
    meta_test_data = np.zeros((ntest, 8))
    models_ls = [models.LogisticRegressionAttacker(), models.MultilayerPerceptronAttacker(), models.RandomForestAttacker(),
                 models.SVMAttacker(), models.GPCAttacker(), models.DTAttacker(), models.AdaBoostAttacker()]
    for idx, attack_type in enumerate([AttackType.LOGISTIC_REGRESSION,
                        AttackType.MULTI_LAYERED_PERCEPTRON,
                        AttackType.RANDOM_FOREST,
                        AttackType.SVM_ATTACK,
                        AttackType.GPC_ATTACK,
                        AttackType.DT_ATTACK,
                        AttackType.ADABOOST_ATTACK]):
        attacker = models_ls[idx]
        attacker.train_model(prepared_attacker_data.features_train,
                             prepared_attacker_data.is_training_labels_train)
        meta_train_data[:, idx] = attacker.predict(prepared_attacker_data.features_train)
        meta_test_data[:, idx] = attacker.predict(prepared_attacker_data.features_test)
        attack_results.append(
            SingleAttackResult(
                slice_spec=_get_slice_spec(attack_input),
                data_size=prepared_attacker_data.data_size,
                attack_type=attack_type,
                roc_curve=metric_attacker_test(prepared_attacker_data.is_training_labels_test, meta_test_data[:, idx]),))
    meta_train_data[:,7] = prepared_attacker_data.features_train[:,-1]
    meta_test_data[:,7] = prepared_attacker_data.features_test[:,-1]
    attack_results.append(_run_threshold_attack(attack_input))

    meta_model = meta_attacker()
    meta_model.fit(meta_train_data, keras.utils.to_categorical(prepared_attacker_data.is_training_labels_train,2) , batch_size=1024, epochs=50, verbose=2)
    print('----- evaluating -----')
    meta_model.evaluate(meta_test_data, keras.utils.to_categorical(prepared_attacker_data.is_training_labels_test,2), verbose=2)
    predictions_test = meta_model.predict(meta_test_data)

    roc_curve = metric_attacker_test(prepared_attacker_data.is_training_labels_test, predictions_test[:,1])


    attack_results.append(SingleAttackResult(
        slice_spec=_get_slice_spec(attack_input),
        data_size=prepared_attacker_data.data_size,
        attack_type=AttackType.META_ATTACK,
        roc_curve=roc_curve,))
...
def meta_attacker():
    """The architecture of meta attacker"""
