...
class RocCurve:
    ...
    def get_precision_recall(self):
        """
        Calculates best precision, recall and F1 at their own best thresholds
        """
        preci = max(self.precision)
        rec = max(self.tpr)
        F1 = (2 * self.precision * self.tpr) / (self.precision + self.tpr)
        F1 = max(F1)
        return preci, rec, F1

    def get_accuracy(self):
        """
        Calculates accuracy at its best threshold
        """
    ...
    def __str__(self):
        ...
        return '\n'.join([
            ...
            '  Precision: %.2f' % self.get_precision_recall()[0],
            '  Recall: %.2f' % self.get_precision_recall()[1],
            '  F1: %.2f' % self.get_precision_recall()[2],
            ')'
        ])
...
class SingleAttackResult:
    ...
    def get_attacker_metric(self):
        adv = self.roc_curve.get_attacker_advantage()
        acc = self.roc_curve.get_accuracy()
        precision, recall, F1 = self.roc_curve.get_precision_recall()
        AP = self.roc_curve.AP
        AUC = self.roc_curve.get_auc()
        d = {}
        d['adv'] = float(adv)
        d['acc'] = float(acc)
    ...
    def get_precision_recall(self):
        return self.roc_curve.get_precision_recall()
...
class AttackResults:
    ...
    def calculate_pd_dataframe(self):
        ...
        for attack_result in self.single_attack_results:
            ...
            precision.append(float(attack_result.get_precision_recall()[0]))
            recall.append(float(attack_result.get_precision_recall()[1]))
            F1.append(float(attack_result.get_precision_recall()[2]))
        df = pd.DataFrame({
            ...
            str(PrivacyMetric.PRECISION): precision,
            str(PrivacyMetric.RECALL): recall,
            str(PrivacyMetric.F1): F1,
        })
        return df
...
def get_averaged_attack_metrics(results: AttackResults):
    """
    Get attack metric
    Args:
    ...
    Returns:
        metric_dict: a dictionary with detailed attack results
        metric_avg_dict: metric_dict averaged across all classes
        metric_avg_all: metric_avg_dict averaged across over all attacker types
        e.g:
        metric_dict['knn']['adv'] = [0.1,0.4,0.1,.....]
        metric_avg_dict['knn']['adv'] = 0.2
        metric_avg_all['adv'] = 0.1
    """
    metric_dict = defaultdict(dict)
    for attack_result in results.single_attack_results:
        if attack_result.attack_type.value not in metric_dict:
            metric_dict[attack_result.attack_type.value] = defaultdict(list)
        attacker_metric = attack_result.get_attacker_metric()

        metric_dict[attack_result.attack_type.value]['adv'].append(
            attacker_metric['adv'])  # advantage is computed from the best threshold
        metric_dict[attack_result.attack_type.value]['auc'].append(attacker_metric['AUC'])
        metric_dict[attack_result.attack_type.value]['precision'].append(attacker_metric['precision'])  # precision
        metric_dict[attack_result.attack_type.value]['recall'].append(
            attacker_metric['recall'])  # recall from threshold of best adv
        metric_dict[attack_result.attack_type.value]['f1'].append(attacker_metric['F1'])  # f1 from precision and recall
        metric_dict[attack_result.attack_type.value]['acc'].append(attacker_metric['acc'])  # threshold 0.5
        metric_dict[attack_result.attack_type.value]['AP'].append(attacker_metric['AP'])
    metric_avg_dict = defaultdict(dict)
    for k, v in metric_dict.items():
        if k not in metric_avg_dict:
            metric_avg_dict[k] = defaultdict(float)
        for vk, vv in v.items():
            metric_avg_dict[k][vk] = sum(vv) / len(vv)  # average over classes

    metric_avg_all = defaultdict(float)
    for k, v in metric_avg_dict.items():
        if k!='meta-attack':
            for kk, vv in v.items():
                metric_avg_all[kk] += vv
    for k in metric_avg_all.keys():
        metric_avg_all[k] /= (len(metric_avg_dict.keys())-1) if 'meta-attack' in metric_avg_dict.keys() else len(metric_avg_dict.keys())

    max_advantage_result_all = results.get_result_with_max_attacker_advantage()
    summary =('%s (with %d training and %d test examples) achieved an advantage of %.2f on slice %s \n'
        % (max_advantage_result_all.attack_type,
           max_advantage_result_all.data_size.ntrain,
           max_advantage_result_all.data_size.ntest,
           max_advantage_result_all.get_attacker_advantage(),
           max_advantage_result_all.slice_spec))
    return metric_dict, metric_avg_dict, metric_avg_all, summary
